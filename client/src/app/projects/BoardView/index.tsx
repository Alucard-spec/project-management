...
const TaskColumn = ({
  ...
  const statusColor: any = {
    "To Do": "bg-[#2563EB]",
    "Work In Progress": "bg-[#059669]",
    "Under Review": "bg-[#D97706]",
    Completed: "bg-[#000000]",
  };
  ...
  return (
    ...
    className={`sl:py-4 rounded-lg py-2 xl:px-2 ${isOver ? "bg-blue-100 " : ""}`}>
      ...
      <div className={`${statusColor[status]} w-2 rounded-s-lg`}/>
        <div className='flex w-full items-center justify-between rounded-e-lg bg-white px-5 py-4 '> 
          <h3 className='flex items-center text-lg font-semibold '>
            {status} {" "}
          
            <span className='ml-2 inline-block rounded-full bg-gray-200 p-1 text-center text-sm leading-none' style={{width:"1.5rem",height:"1.5rem"}}>
              {tasksCount}
            </span>
            </h3>
            <div className='flex items-center gap-1 '>
              ...
              <button className='flex h-6 w-6 items-center justify-center rounded bg-gray-200' onClick={()=> setIsModalNewTaskOpen(true)}>
              <Plus size={16}/>
              </button>  
            </div>
        </div>
